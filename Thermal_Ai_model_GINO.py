...
from Resources.losses import RelativeLoss
from Resources.temp_dataset import SingleStepMultiTempDataset1D
from Resources.ai_models import GINO
...
write = SummaryWriter("runs/GINO/GINO_" + str(datetime.datetime.now().strftime("%y%m%d_%H%M")))
...
stopping_path = "logs/EarlyStopping.pth"
stop_path = os.path.join(absolute_path, stopping_path)
earlyStopping = EarlyStopping(start_epoch=400, path=stop_path, patience=150)

model = GINO(start_channels=start_channels, out_channels=1, fourier_modes=16, width=64, radius=0.01).to(device)
...
for epoch in range(epochs):
    ...
    # encoder.eval()
    with torch.no_grad():
        with tqdm(testing_dataloader2, desc="Processing", leave=False) as pbar:
            for x, y in pbar:
                x_list, y = x, y
                x = x_list[0]
                x, y = x.to(device, dtype=torch.float32), y.to(device, dtype=torch.float32)
...
x = next(iter(testing_dataloader))[0][0]
